...
Sub Globals
	...
	Private cust_LV_ayabzahab As CustomListView
...
Sub Activity_Create(FirstTime As Boolean)
	...
	Activity.LoadLayout("ayabzahab_layout")
	...
	fill_list_ayabzahab(sp_year.SelectedItem,moon_num)
...
Sub fill_list_ayabzahab(year As String , moon As String)
	
	''--------- لیست مساعده ها -------------
	cust_LV_ayabzahab.Clear
	'list_ezafekari_id.Clear
	
	dbCode.connect_db

	dbCode.res= dbCode.sql.ExecQuery("SELECT * FROM tb_ayabzahab WHERE date LIKE '%"&year&"/"&moon&"%' ORDER BY  date DESC;")
	...
	Do While dbCode.res.NextRow
		...
		cust_LV_ayabzahab.Add(p,dbCode.res.GetString("id"))
	...
	If(cust_LV_ayabzahab.LastVisibleIndex<5)Then
		p = xui2.CreatePanel("p")
		Select cust_LV_ayabzahab.LastVisibleIndex
			Case -1
...
Private Sub lbl_remove_from_list_Click
	...
	If result = DialogResponse.Positive Then
		...
		If (dbCode.delete_ayabZahab(b.Tag))Then
			ToastMessageShow("حذف شد.",False)
			
			'TabHost2.CurrentTab=2
			fill_list_ayabzahab(sp_year.SelectedItem,myfunc.convert_adad(sp_moon.SelectedIndex+1))
...
Private Sub lbl_edit_from_list_Click
	...
	dbCode.res= dbCode.sql.ExecQuery("SELECT * FROM tb_ayabzahab WHERE id="&current_itemId_edit)
	...
	Else
		...
		If(addEdit_id=0)Then
			...
			If(sp_type_state.SelectedIndex=0)Then
				dbCode.add_aybZahab(et_onvan_edit1.Text,lbl_date_edit1.Text,et_mablagh_edit1.Tag,et_tozih_edit1.Text,1)
			Else
				dbCode.add_aybZahab(et_onvan_edit1.Text,lbl_date_edit1.Text,et_mablagh_edit1.Tag,et_tozih_edit1.Text,2)
			End If
			ToastMessageShow("اضافه شد",False)
		Else If(addEdit_id=1)Then
			If(sp_type_state.SelectedIndex=0)Then
				dbCode.edit_ayabZahab(current_itemId_edit,et_onvan_edit1.Text,lbl_date_edit1.Text,et_mablagh_edit1.Tag,et_tozih_edit1.Text,1)
			Else
				dbCode.edit_ayabZahab(current_itemId_edit,et_onvan_edit1.Text,lbl_date_edit1.Text,et_mablagh_edit1.Tag,et_tozih_edit1.Text,2)
			End If
...
Private Sub lbl_add_ayabzahab_Click
