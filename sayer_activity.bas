...
Private Sub lbl_share_Click
	...
	name_gozaresh="سایر-"
	name_gozaresh_en="sayer"
	dbCode.res= dbCode.sql.ExecQuery("SELECT * FROM tb_sayer WHERE date LIKE '%"&myfunc.fa2en(sp_year.SelectedItem)&"/"&myfunc.convert_adad(sp_moon.SelectedIndex+1)&"%' ORDER BY  date DESC;")
