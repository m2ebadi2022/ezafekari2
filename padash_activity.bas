...
Sub Activity_Create(FirstTime As Boolean)
	...
	fill_list_padash(sp_year.SelectedItem,moon_num)
...
Private Sub lbl_save_edit1_Click
	...
	Else
		...
		If(addEdit_id=0)Then
			dbCode.add_padash(et_onvan_edit1.Text ,lbl_date_edit1.Text,et_mablagh_edit1.Tag,et_tozih_edit1.Text,0)
...
Private Sub lbl_add_padash_Click
	
	addEdit_id=0
	item_edit_box_mod("ثبت پاداش","",myfunc.fa2en(Main.persianDate.PersianShortDate),"0","",0)
End Sub
...
Private Sub lbl_share_Click
	...
	Dim majmoe_mablagh As Int = 0
	
	dbCode.connect_db
	

	name_gozaresh="پاداش ها-"
	name_gozaresh_en="padash"
	dbCode.res= dbCode.sql.ExecQuery("SELECT * FROM tb_padash WHERE date LIKE '%"&myfunc.fa2en(sp_year.SelectedItem)&"/"&myfunc.convert_adad(sp_moon.SelectedIndex+1)&"%' ORDER BY  date DESC;")
	...
	str_file_matn.Append("<td><b> ردیف</b></td><td><b> تاریخ</b></td><td><b> عنوان</b></td><td><b> مبلغ</b></td><td><b> توضیح</b></td><br></tr>")
	...
	Do While dbCode.res.NextRow
		...
		majmoe_mablagh=majmoe_mablagh+dbCode.res.GetString("mablagh")
		str_file_matn.Append("</tr>")
	Loop
		
		
	str_file_matn.Append("</table>")
	str_file_matn.Append("مجموع مبلغ  :<span style='color:green;'> "& majmoe_mablagh &" </span><br></div><br> ")
