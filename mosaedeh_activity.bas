...
Private Sub lbl_share_Click
	...
	dbCode.connect_db
			...
			name_gozaresh="مساعده ها-"
			name_gozaresh_en="mosaedeha"
	dbCode.res= dbCode.sql.ExecQuery("SELECT * FROM tb_mosaedeh WHERE date LIKE '%"&myfunc.fa2en(sp_year.SelectedItem)&"/"&myfunc.convert_adad(sp_moon.SelectedIndex+1)&"%' ORDER BY  date DESC;")
	...
	str_file_matn.Append("مجموع مبلغ  :<span style='color:#5E35B1;'> "& majmoe_mablagh &" </span><br></div><br> ")
