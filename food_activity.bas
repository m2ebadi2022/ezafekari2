...
Sub Activity_Create(FirstTime As Boolean)
	...
	Activity.LoadLayout("food_layout")
	...
	fill_list_food(sp_year.SelectedItem,moon_num)
...
Private Sub lbl_add_food_Click
	
	addEdit_id=0
	item_edit_box_mod("ثبت هزینه غذا","",myfunc.fa2en(Main.persianDate.PersianShortDate),"0","",0)
End Sub
...
Private Sub lbl_share_Click
	...
	name_gozaresh="هزینه غذا-"
	name_gozaresh_en="food"
	dbCode.res= dbCode.sql.ExecQuery("SELECT * FROM tb_food WHERE date LIKE '%"&myfunc.fa2en(sp_year.SelectedItem)&"/"&myfunc.convert_adad(sp_moon.SelectedIndex+1)&"%' ORDER BY  date DESC;")
